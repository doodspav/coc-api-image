...
#change name and clan name to not show blanks
    def main_picture_info(self):
        ...
        for i in range(4):
            ...
            y = bn_y - width
            draw.text((bn_x[i],y), str(numbers[i]), white, font=bn_font)
        ...
        #current cups
        cc_font = ImageFont.truetype("cocapifiles/fonts/Supercell-Magic_5.ttf", 36)
        cc_x, cc_y = 1340,139
        ...
        if self.in_clan == True:
            ...
            role_img = Image.open("cocapifiles/role/"+role_dict[self.role]+".png")
...
#make legends badges nicer
    def main_picture_legends(self):
        ...
        draw.text((bs_x+2,bx_y), str(self.best_season_trophies), black, font=bs_font)
...
#find out what level_font is (size and font)
    def main_picture_troops(self, troop_levels=None, spell_levels=None, hero_levels=None):
        ...
        if troop_levels == None:
            troops_levels = self.main_troop_levels
        for i in range(self.main_troop_number):
            if troop_levels[i] != 0:
                ...
                if troop_levels[i] != 1:
                    ...
                    w, h = math.floor(width), math.floor(height)
                    x, y = level_font.getsize(str(troop_levels[i]))
                    x, y = (w-x),(h-y) #top left of the number
                    draw = ImageDraw.Draw(level_img)
                    draw.text((x+1,y), troop_levels[i], black, font=level_font)
                    draw.text((x-1,y), troop_levels[i], black, font=level_font)
                    draw.text((x,y-1), troop_levels[i], black, font=level_font)
                    draw.text((x,y+3), troop_levels[i], black, font=level_font)
                    draw.text((x,y), troop_levels[i], white, font=level_font)
                    troopimg2 = troopimg.crop(box=(5,44,(5+w),(44+h)))
                    troopimg3 = Image.alpha_composite(troopimg2, troopimg)
                    troopimg.paste(troopimg3, box=(5,44))
        ...
        for i in range(self.main_spell_number):
            if spell_levels[i] != 0:
                ...
                if spell_levels[i] != 1:
                    ...
                    x, y = (w-x),(h-y) #top left of the number
                    draw = ImageDraw.Draw(level_img)
                    draw.text((x+1,y), spell_levels[i], black, font=level_font)
                    draw.text((x-1,y), spell_levels[i], black, font=level_font)
                    draw.text((x,y-1), spell_levels[i], black, font=level_font)
                    draw.text((x,y+3), spell_levels[i], black, font=level_font)
                    draw.text((x,y), spell_levels[i], white, font=level_font)
                    spellimg2 = spell_img.crop(box=(5,44,(5+w),(44+h)))
                    spellimg3 = Image.alpha_composite(spellimg2, spell_img)
        ...
        for i in range(self.main_hero_number):
            if hero_levels[i] != 0:
                ...
                x, y = (w-x),(h-y) #top left of the number
                draw = ImageDraw.Draw(level_img)
                draw.text((x+1,y), hero_levels[i], black, font=level_font)
                draw.text((x-1,y), hero_levels[i], black, font=level_font)
                draw.text((x,y-1), hero_levels[i], black, font=level_font)
                draw.text((x,y+3), hero_levels[i], black, font=level_font)
                draw.text((x,y), hero_levels[i], white, font=level_font)
                heroimg2 = hero_img.crop(box=(5,44,(5+w),(44+h)))
                heroimg3 = Image.alpha_composite(heroimg2, hero_img)
