...
class Player:
    ...
    def builder_picture_info(self):
        ...
        if self.in_clan == True:
            ...
            cn_font = ImageFont.truetype("cocapifiles/fonts/Supercell-Magic_5.ttf", 26)
            cn_x, cn_y = 124,126
            draw = self.draw_outline_shadow(draw, (cn_x,cn_y), (2,5,2,2), self.clan_name, cn_font)
            #clan role
            role_dict = {"admin":"Elder", "member":"Member", "coLeader":"Co-leader", "leader":"Leader"}
            role_img = Image.open("cocapifiles/role/"+role_dict[self.clan_role]+".png")
    ...
    def main_full_profile(self, player_tag, path="test.jpg", ext="JPEG"):
        ...
        if self.status_code == 200:
            ...
            #processes
            mpi_p = pool.apply_async(self.main_picture_info)
            mpt_p = pool.apply_async(self.main_picture_troops)
            if self.is_legend:
                mpl_p = pool.apply_async(self.main_picture_legends)
            #dicts
            mpi_d = mpi_p.get()
            mpt_d = mpt_p.get()
            if self.is_legend:
                mpl_d = mpl_p.get()
            #image objects
            mpi = Image.frombytes(mpi_d['mode'], mpi_d['size'], mpi_d['data'])
            mpt = Image.frombytes(mpt_d['mode'], mpt_d['size'], mpt_d['data'])
            if self.is_legend:
                mpl = Image.frombytes(mpl_d['mode'], mpl_d['size'], mpl_d['data'])
            #sizes
